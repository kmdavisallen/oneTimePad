...
int main(int argc, char *argv[])
{
	...
	if (strcmp(buffer, "The kind of thing an idiot would have on his luggage") != 0) {
		fprintf(stderr, "cannot use otp_enc with otp_dec_d\n");
		exit(2);
	}

	//open messge from file
	int size = 0;
	...
	memset(message, '\0', size); // Clear out the message again for reuse
	charsRead = 0;
